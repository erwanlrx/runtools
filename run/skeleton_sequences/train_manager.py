...
for idx in product(*idxs):
    argv1 = main_argv(*idx)
    argv1.extend(['run_prefix=' + run_prefix, 'training_steps=100', 'summary_flush_rate=10', 'checkpoint_rate=100'])
    ...
    for batch_size, learning_rate in product([128], [1e-2]):
        ...
        for rnn_units, rnn_layers, rnn_type in product([100], [1], ['lstm', 'gru']):
            argv3 = argv2[:]
